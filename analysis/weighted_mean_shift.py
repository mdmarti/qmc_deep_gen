...
class WeightedMeanShift(MeanShift):
    ...
    def __init__(
        ...
        *,
        ...
        super(WeightedMeanShift, self).__init__(
            bandwidth=bandwidth,
            seeds=seeds,
    ...
    def fit(
        ...
        all_res = Parallel(n_jobs=self.n_jobs)(
            delayed(_mean_shift_single_seed)(
                seed, X, nbrs, self.max_iter, weights, no, verbose
            )
        ...
        nbrs = NearestNeighbors(
            radius=bandwidth, n_jobs=self.n_jobs, metric=self.metric
        ).fit(sorted_centers)
        ...
        nbrs = NearestNeighbors(
            n_neighbors=1, n_jobs=self.n_jobs, metric=self.metric
        ).fit(cluster_centers)
    ...
    def predict(
        ...
        y=None,
        weights: Union[None, np.ndarray] = None,
        weight_grid: Union[None, np.ndarray] = None,
        verbose: bool = False,
        tol: float = 1e-2,
        max_iter: int = 1000,
        bandwidth: float = 0.1,
        ...
        if weights is not None:
            if weight_grid is None:
                assert X.shape[0] == len(weights), print(
                    "If you do not provide a weight grid, weights must be over X!"
                )
                weight_grid = X
            else:
                assert len(weights) == weight_grid.shape[0], print(
                    "Weight grid must have same length as weights!"
                )
        nbrs = NearestNeighbors(radius=bandwidth, n_jobs=1, metric=self.metric).fit(
            weight_grid
        )
        self.nbrs = nbrs
...
def _predict_single_seed(
    my_mean, X, nbrs, max_iter, centers, weights=None, tol=1e-2, verbose=False
):
    ...
    while True:
        ...
        my_mean = np.sum(
            weights_within * points_within, axis=0
        )  # np.mean(points_within*weights_within, axis=0)
        # print(my_mean)
        # If converged or at max_iter, adds the cluster
        if np.any(np.linalg.norm(my_mean[None, :] - centers, axis=1) <= stop_thresh):
            # print(completed_iterations)
            label = np.argmin(
                np.linalg.norm(my_mean[None, :] - centers, axis=1)
            ).squeeze()
            break
    ...
    if verbose:
        print(f"finished seed in {completed_iterations} iterations, {time_len}s")
    # assert False
...
def _mean_shift_single_seed(
    my_mean, X, nbrs, max_iter, weights=None, seed_no=0, verbose=False
):
    ...
    if verbose:
        print(
            f"finished seed {seed_no} in {completed_iterations} iterations, {time_len}s"
        )
    # assert False
...
class WeightedMeanShiftCircular(WeightedMeanShift):
    ...
    def __init__(
        ...
        *,
        ...
        super(WeightedMeanShiftCircular, self).__init__(
            bandwidth=bandwidth,
            seeds=seeds,
            bin_seeding=bin_seeding,
            min_bin_freq=min_bin_freq,
    ...
    def _mean_shift_single_seed(
        ...
        while True:
            ...
            my_mean = np.sum(
                weights_within * wrapped_points, axis=0
            )  # np.mean(points_within*weights_within, axis=0)
            # If converged or at max_iter, adds the cluster
            if (
                np.linalg.norm(my_mean - my_old_mean) <= stop_thresh
