...
def run_mean_shift(
    ...
    np.ndarray,
    Union[WeightedMeanShift, WeightedMeanShiftCircular],
    ...
    if embedded:
        ...
        wms = WeightedMeanShift(
            n_jobs=n_jobs, bandwidth=2 * bandwidth, metric=metric, seeds=seeds
        )
    else:
        metric = lambda x, y: p_dist_theta(x, y, p=p)
        print("using circular metric")
        wms = WeightedMeanShiftCircular(
            n_jobs=n_jobs, bandwidth=bandwidth, metric=metric, seeds=seeds
        )
