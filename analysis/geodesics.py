...
def prob_ratio(
    x: torch.FloatTensor, y: torch.FloatTensor, eps: float = 1e-15
) -> torch.FloatTensor:
    """
...
def get_minimizing_curve(grid, posterior, p0, p1):
    ...
    c, _ = manif.connecting_geodesic(
        torch.from_numpy(p0).to(torch.float32), torch.from_numpy(p1).to(torch.float32)
    )
...
def run_dijkstra(
    lattice: np.ndarray, node1_ind: int, node2_ind: int, graph: np.ndarray
    ...
    dists, predecessors = shortest_path(
        graph, directed=True, indices=node1_ind, return_predecessors=True
    )
    path = []
    ii = node2_ind
    while ii != node1_ind:
...
def get_linear_path(
    point_1: np.ndarray, point_2: np.ndarray, path_len: int
) -> np.ndarray:
    """
