...
def model_grid_plot(model,n_samples_dim,fn='',show=True):
    ...
    with torch.no_grad():
        ...
        z = torch.stack([xx.flatten(),yy.flatten()],axis=-1).to(model.device)
    ...
    mosaic = [[['scatter grid']*n_samples_dim]*n_samples_dim,
              [[f"sample {ii}" for ii in range(jj*n_samples_dim,(jj+1)*n_samples_dim)] for jj in range(n_samples_dim)]
               ]
