...
def model_grid_plot(
    model, n_samples_dim, fn="", show=True, origin=None, cm="grey", model_type="qmc"
):
    ...
    with torch.no_grad():
        ...
        xx, yy = torch.meshgrid(
            [torch.linspace(EPS1, 1 - EPS2, n_samples_dim)] * 2, indexing="ij"
        )
        z = torch.stack([xx.flatten(), yy.flatten()], axis=-1).to(device)
        if model_type == "vae":
            dist = stats.norm
            z = (
                torch.from_numpy(dist.ppf(z.detach().cpu().numpy()))
                .to(torch.float32)
                .to(model.device)
            )
    ...
    mosaic = np.flipud(
        [
            [f"sample {jj * n_samples_dim + ii}" for ii in range(n_samples_dim)]
...
def conditional_qmc_grid_plot(
    model, n_samples_dim, c, fn="", show=True, origin=None, cm="grey"
):
...
def vae_train_plot(vae_train_losses, vae_test_losses, save_fn):
    ...
    vae_train_recons, vae_train_kls = (
        np.array(vae_train_recons),
        np.array(vae_train_kls),
    )
    ...
    (l2,) = ax.plot(
        -vae_train_recons, label="VAE reconstruction log probability", color="tab:blue"
    )
    (l3,) = ax.plot(
        -vae_train_recons - vae_train_kls, label="VAE ELBO", color="tab:green"
    )
    ...
    ax.errorbar(
        N + N / 100,
        np.nanmean(-vae_test_recons),
        yerr=np.nanstd(-vae_test_recons),
        color="tab:blue",
        capsize=6,
        linestyle="",
    )
    ax.errorbar(
        N + N / 100,
        np.nanmean(-vae_test_kls),
        yerr=np.nanstd(-vae_test_kls),
        color="tab:orange",
        capsize=6,
        linestyle="",
    )
    ax.errorbar(
        N + N / 100,
        np.nanmean(-vae_test_recons - vae_test_kls),
        yerr=np.nanstd(-vae_test_recons - vae_test_kls),
        color="tab:green",
        capsize=6,
        linestyle="",
    )
    ax = format_plot_axis(
        ax, ylabel="", xlabel="update number", xticks=xax, xticklabels=xticklabels
    )
...
def qmc_train_plot(qmc_train_losses, qmc_test_losses, save_fn="", show=False):
    ...
    qmc_train_losses, qmc_test_losses = (
        np.array(qmc_train_losses),
        np.array(qmc_test_losses),
    )
    ...
    ax.errorbar(
        N + N // 5,
        np.nanmean(-qmc_test_losses),
        yerr=np.nanstd(-qmc_test_losses),
        capsize=6,
        linestyle="",
        color="tab:blue",
    )
...
def round_trip_qmc(model, grid, data, log_density, device="cuda"):
    ...
    posterior = (
        model.posterior_probability(grid.to(device), data.to(device), log_density)
        .detach()
        .cpu()
    )
...
def recon_comparison_plot(
    ...
    loader,
    qmc_lattice,
    n_samples_comparison=6,
    save_path="recon_{sample_num}.png",
    cm="gray",
    origin=None,
    show=False,
    ...
    sample_inds = np.random.choice(
        len(loader.dataset), n_samples, replace=False
    ).squeeze()
    mosaic = [
        [f"qmc {ii}" for ii in range(n_samples)],
    ...
    for ii, sample_ind in tqdm(enumerate(sample_inds), total=len(sample_inds)):
        ...
        sample = (
            torch.tensor(loader.dataset[sample_ind][0])
            .to(torch.float32)
            .to(qmc_model.device)
        )
        if sample.shape[0] == 1:
        ...
        recon_qmc = (
            qmc_model.round_trip(
                qmc_lattice,
                sample,
                qmc_likelihood,
                recon_type=recon_type,
                n_samples=n_samples_recon,
            )
            .detach()
            .cpu()
        )
        ...
        if "mocap" in save_path.lower():
            ...
            axes[f"sample {ii}"] = plot_motion_on_ax(
                axes[f"sample {ii}"], motion, joints
            )

            recon_qmc = (
                recon_qmc.detach().cpu().numpy().squeeze().mean(axis=0, keepdims=True)
            )
            ...
            recon_vae = (
                recon_vae.detach().cpu().numpy().squeeze().mean(axis=0, keepdims=True)
            )
...
def sample_comparison_plot(
    ...
    vae_model,
    n_samples_comparison=6,
    save_path="samples_{sample_num}.png",
    cm="gray",
    origin=None,
    show=False,
    loader=[],
    ...
    for ii, sample_ind in tqdm(enumerate(range(n_samples)), total=n_samples):
        if "mocap" in save_path.lower():
            ...
            joints = loader.dataset.joints
        # save_path_ind = save_path.format(sample_num = sample_ind)
        qmc_sample = torch.rand(
            (1, qmc_model.latent_dim), device=qmc_model.device, dtype=torch.float32
        )
        vae_sample = (
            torch.normal(mean=0, std=1, size=(1, qmc_model.latent_dim))
            .to(torch.float32)
            .to(vae_model.device)
        )
        ...
        if "mocap" in save_path.lower():
            # print("using this version")
            recon_iwae = (
                recon_iwae.detach().cpu().numpy().squeeze().mean(axis=0, keepdims=True)
            )
...
def posterior_comparison_plot(
    ...
    n_points=50,
    show=False,
    save_path="posterior_{sample_num}.png",
):
    ...
    for sample_ind in tqdm(sample_inds):
        ...
        sample = (
            sample.view(1, 1, sample.shape[-2], sample.shape[-1])
            .to(torch.float32)
            .to(vae_model.device)
        )
        emp_posterior, enc_posterior, grid = vae_model.posterior(
            sample, n_points, log_prob
        )
        ...
        recon_post_emp = (
            vae_model.decoder(
                torch.from_numpy(emp_weighted_grid)
                .view(1, 2)
                .to(torch.float32)
                .to(vae_model.device)
            )
        ...
        recon_post_enc = (
            vae_model.decoder(
                torch.from_numpy(prop_weighted_grid)
                .view(1, 2)
                .to(torch.float32)
                .to(vae_model.device)
            )
        ...
        axs["Post1"] = format_plot_axis(
            axs["Post1"],
            title="Empirical posterior\n(based on decoder)",
            xticks=axs["Post1"].get_xticks(),
            yticks=axs["Post1"].get_yticks(),
        )
        axs["Post2"] = format_plot_axis(
            axs["Post2"],
            title="Encoder proposal distribution",
            xticks=axs["Post2"].get_xticks(),
            yticks=axs["Post2"].get_yticks(),
        )

        axs["Recon1"].imshow(recon_post_emp, cmap="gray")
        axs["Recon1"] = format_img_axis(
            axs["Recon1"], title="Recon from\nempirical posterior"
        )
...
def round_trip_plot_qmc(
    qmc_model,
    qmc_likelihood,
    loader,
    qmc_lattice,
    n_samples_comparison=6,
    save_path="recons_qmc.png",
    cm="gray",
    origin=None,
    show=False,
    recon_type="posterior",
    ...
    sample_inds = np.random.choice(
        len(loader.dataset), n_samples_comparison, replace=False
    )

    mosaic = [
        [f"qmc {ii}" for ii in range(n_samples_comparison)],
...
def round_trip_plot_vae(
    vae_model,
    loader,
    n_samples_comparison=6,
    save_path="recons_vae.png",
    cm="gray",
    origin=None,
    show=False,
    recon_type="posterior",
    ...
    mosaic = [
        [f"vae {ii}" for ii in range(n_samples_comparison)],
        [f"sample {ii}" for ii in range(n_samples_comparison)],
    ]
    fig, axes = plt.subplot_mosaic(
        mosaic,
    ...
    axes["vae 0"].set_ylabel(
        f"{vae_model.encoder.latent_dim}d Lattice-LVM Reconstructions"
    )
    axes["sample 0"].set_ylabel("Original samples")
    if show:
        plt.show()
