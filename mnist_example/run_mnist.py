...
def run_mnist(dataloc, save_plots=False, max_workers=4, batch_size=128):
    ...
    print("Training QLVM")
    qlvm, qlvm_opt, qlvm_losses = train_loop(
        qlvm, train_loader, qmc_lattice.to(qlvm.device), qlvm_loss, nEpochs=20
    )
    qlvm_test_loss = test_epoch(
        qlvm, test_loader, qmc_lattice.to(qlvm.device), qlvm_loss
    )
    print("Training VAE")
    ...
    print("Training IWAE")
    iwae, iwae_opt, iwae_losses = train_loop_vae(
        iwae, train_loader, iwae_loss, nEpochs=20
    )
