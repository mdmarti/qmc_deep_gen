...
def train_loop_vae(model, loader, loss_function, nEpochs=100):
    ...
    for epoch in tqdm(range(nEpochs)):
        model, optimizer, batch_recon, batch_kl = train_epoch_vae(
            model, optimizer, loader, loss_function
        )
...
def test_epoch(model, loader, base_sequence, loss_function, random=True, mod=True):
...
def train_loop(
    model, loader, base_sequence, loss_function, nEpochs=100, random=True, mod=True
):
