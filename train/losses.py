...
def binary_evidence(
    samples, data, reduce=True, batch_size=-1, importance_weights=[], full=True
):
    ...
    for batch_start in range(0, samples.shape[0], batch_size):
        ...
        rl = binary_lp(
            samples[batch_start:batch_end], data, importance_weights=importance_weights
        )
        recon_loss.append(rl)
    recon_loss = torch.cat(recon_loss, axis=1)
    recon_loss = torch.special.logsumexp(recon_loss, axis=1)
    if full:
...
def binary_evidence_old(samples, data, reduce=True, batch_size=-1):
    ...
    for start_ind in range(0, B, batch_size):
        ...
        rl = torch.sum(binary_lp_old(samples[start_ind:end_ind], data), axis=(2, 3))
        recon_loss.append(rl)
    ...
    try:
        ...
        if len(importance_weights) == 0:
            importance_weights = torch.ones(
                (1, K), device=samples.device, dtype=torch.float32
            )
...
def gaussian_evidence(
    samples, data, var, reduce=True, batch_size=-1, importance_weights=[], full=True
):
...
def gaussian_evidence_old(samples, data, var=1.0, reduce=True, batch_size=-1):
    ...
    for start_ind in range(0, B, batch_size):
        ...
        rl = torch.sum(
            gaussian_lp_old(samples[start_ind:end_ind], data, var), axis=(2, 3)
        )
        recon_loss.append(rl)
...
def gaussian_evidence_with_blur(samples, data, var=1.0, kernel_size=4, sigma=0.25):
...
def gaussian_lp(samples, data, var, importance_weights=[]):
    ...
    vmapped_lp = torch.vmap(
        torch.vmap(gaussian_nll_loss, in_dims=(0, None)), in_dims=(None, 0)
    )
    return -vmapped_lp(samples, data, var=var, reduction="sum", full=True) + torch.log(
        importance_weights
    )  # since this should be log(p(x|z)p(z))
...
def gaussian_elbo(reconstructions, distribution, targets, recon_precision=1e-2, beta=1):
    ...
    neg_lp = gaussian_nll_loss(
        reconstructions, targets, var=1 / recon_precision, reduction="none", full=True
    ).sum(axis=(1, 2, 3))
    # torch.einsum('bchw,bchw->b',err,err) *(recon_precision)/2 + \
    # d*np.log(2*torch.pi)/2 - d * np.log(recon_precision)/2

    t12 = -1 / 2 * torch.log(D).sum(dim=-1) - 1 / 2 * torch.log(
        (1 + torch.einsum("bd,bd->b", L / D, L))
    )  # torch.log(torch.prod(D,dim=-1)*(1 + torch.einsum('bd,bd->b',L/D,L)))
...
def gaussian_iwae_elbo(reconstructions, distribution, targets, recon_precision=1e-2):
    ...
    prior_ll = (
        -torch.einsum("kbd,kbd->bk", z, z) / 2 - d * np.log(2 * np.pi) / 2 - d / 2
    )  ## needs to also be B x k

    ### finally, learned latent dist ll
    latent_ll = dist.log_prob(z).permute(1, 0)  # should be B x k

    # return neg_lp.mean(),kl.mean()
    return -torch.special.logsumexp(recon_ll + prior_ll - latent_ll, dim=1).mean(
        dim=0
    ), torch.tensor([0.0]).to(reconstructions.device)
...
def binary_elbo(reconstructions, distribution, targets, beta=1):
    ...
    neg_lp = binary_cross_entropy(reconstructions, targets, reduction="none").sum(
        axis=(1, 2, 3)
    )
...
def kl_tests():
    ...
    assert torch.isclose(kl_nonones_diag, torch.Tensor([13 - np.log(48) - 4]) / 2), (
        print(
            kl_nonones_diag,
            torch.Tensor([torch.sum(d) - torch.sum(torch.log(d)) - 4]) / 2,
        )
    )
    ...
    mu = torch.Tensor([[0, 0], [1, 2], [0, 0], [0, 0]])
    L = torch.Tensor([[0, 0], [0, 0], [0, 0], [1, 2]])
    d = torch.Tensor([[1, 1], [1, 1], [1, 4], [3, 1]])
    kl_batch = kl_term(mu, L, d)
    expected = torch.Tensor([0.0, 5, 5 - np.log(4) - 2, 9 - np.log(16) - 2]) / 2
