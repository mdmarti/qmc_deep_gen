...
def load_segmented_sylls(bird_filepath, sylls, test_size=0.2, seed=92):
    ...
    train_files, test_files, train_ids, test_ids = train_test_split(
        spec_files, syll_ids, test_size=test_size, random_state=seed
    )
...
def load_gerbils(gerbil_filepath, families=[2], test_size=0.2, seed=92, check=True):
    ...
    if check:
        ...
        assert len(num_specs) == 1, print(
            f"Files have different numbers of specs in them! {num_specs}"
        )
        if num_specs[0] != specs_per_file:
            print(
                f"expected {specs_per_file} specs per file, found {num_specs[0]}; updating"
            )
    ...
    if test_size > 0:
        ...
        for family in families:
            ...
            tr_fn, te_fn, tr_id, te_id = train_test_split(
                specs, ids, test_size=test_size, random_state=seed
            )
...
def calc_fm(spec):
    ...
    fm = (
        np.arctan2(dt2, df2[:-1]) / np.pi
    )  # divide by pi to make sure this is between -1,1
