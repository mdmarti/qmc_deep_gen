    ...
    for j in joints.keys():
        ...
        if not drop:
            if joint.name == "root":
                ...
                globalrots.append(
                    np.hstack([np.sin(globalRot), np.cos(globalRot)])
                )  # SHOULD be size 2xDoF
            else:
                idx = 0
                rotation = []
...
def sample2motion(sample, motion, conversion_key, testing=False):
    ...
    for sampleFrame, motionFrame in zip(sample, motion):
        ...
        for jointName, DoF in conversion_key:
            ...
            if jointName == "root":
                newFrame[jointName] = (
                    motionFrame[jointName][:3] + degrees_recon.tolist()
                )  # unsure if this will work, but it should
                if testing:
                    assert np.all(
                        np.isclose(np.array(motionFrame[jointName][3:]), degrees_recon)
                    ), print(
                        motionFrame[jointName][3:],
                        degrees_recon.tolist(),
                        start_ind,
                        start_ind + DoF,
                    )
            else:
                ...
                if testing:
                    assert np.all(
                        np.isclose(degrees_recon, np.array(motionFrame[jointName]))
                    ), print(
                        degrees_recon,
                        np.array(motionFrame[jointName]),
                        start_ind,
                        start_ind + DoF,
                    )
...
def plot_sample(sample, joints, motion, conversion_key, n_per_sample=5):
    ...
    fig, axs = plt.subplots(
        nrows=1, ncols=n_per_sample, subplot_kw=dict(projection="3d"), figsize=(20, 5)
    )
...
def get_samples(datapath, subject, n_frames_per_sample=4, test_size=0.2, seed=92):
    ...
    for ii, trial in tqdm(enumerate(frames), total=len(frames)):
        traj, mean, key = preprocess_mocap_motion(
            joints, trial, n_frames_per_sample=n_frames_per_sample
        )
    ...
    trials, labels, frame_nos = (
        np.vstack(trials),
        np.hstack(labels),
        np.hstack(frame_nos),
    )
    train_trials, test_trials, train_labels, test_labels, train_frames, test_frames = (
        train_test_split(
            trials, labels, frame_nos, test_size=test_size, random_state=seed
        )
    )
...
class MocapDataset(Dataset):
    ...
    def __getitem__(self, index, return_all_info=False):
        ...
        sample = self.transform(sample).unsqueeze(
            0
        )  # ,(1,sample.shape[0],sample.shape[1])))#.unsqueeze(0)
...
def model_grid_plot(
    ...
    mosaic = [
        [f"sample {ii * n_samples_dim + jj}" for ii in range(n_samples_dim)]
