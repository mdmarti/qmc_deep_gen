...
def plot_mocap_gif(joint, motion, latents=[], fn="testani.mp4"):
    ...
    if len(latents) == 0:
        fig, jointAx = plt.subplots(
            nrows=1, ncols=1, subplot_kw=dict(projection="3d"), layout="compressed"
        )
        ...
        fig, (jointAx, latAx) = plt.subplots(
            nrows=1, ncols=2, subplot_kw=dict(projection="3d"), layout="compressed"
        )
    ...
    ani = animation.FuncAnimation(
        fig, anim, frames=len(motion) * 3, interval=50, blit=True
    )
...
def getRotation(
    joints, motion, excluded=["toes", "hand", "fingers", "thumb", "hipjoint"]
):
...
def preprocess_mocap(jointList, motionsList):
    ...
    for subject, motions in zip(jointList, motionsList):
        ...
        for trial in motions:
            ...
            globalTranslation = np.vstack(
                [globalTranslation, globalTranslation[-1:, :]]
            )
...
class Lorenz96(ToyData):
    ...
    def dW(self, dt):
        return self.gen.multivariate_normal(
            mean=np.zeros((self.d,)), cov=np.eye(self.d) * dt
        )
...
class DoubleSDE(ToyData):
    def __init__(
        self, coeffs=[np.pi, -np.pi, np.array([-0.5, 0]), np.array([0.5, 0])], seed=1234
    ):
...
class Balls(ToyData):
    ...
    def traj_to_movie(self, trajectories, image_shape, radius, blur=False):
        ...
        movies = []
        gX, gY = np.meshgrid(
            np.linspace(-1, 1, 2 * radius), np.linspace(-1, 1, 2 * radius)
        )
        ball = gX**2 + gY**2 < 1
        ...
        for traj in trajectories:
            ...
            for point in range(len(traj)):
                ballCenter = (traj[point, :] * (image_shape - 2 * radius)).astype(
                    int
                ) + radius
                frames[
                    point,
                    0,
                    ballCenter[0] - radius : ballCenter[0] + radius,
                    ballCenter[1] - radius : ballCenter[1] + radius,
                ] += ball
                if blur:
                    frames[point, 0, :, :] = gaussian_filter(
                        frames[point, 0, :, :], sigma=radius * 4, truncate=0.05
                    )
...
def downsample(
    data: list, origdt: float, newdt: float, noise: bool = True
) -> np.ndarray:
...
class toyDataset(Dataset):
    def __init__(self, data, dt, nForward=1) -> None:
        ...
        sets = [
            np.vstack(
                [
                    np.arange(ii, l + ii - self.maxForward)
                    for ii in range(self.maxForward + 1)
                ]
            ).T
            for l in lens
        ]
...
class toyDatasetLinearity(Dataset):
    def __init__(self, data, dt) -> None:
        ...
        lens2 = [0] + list(np.cumsum([l for l in lens][:-1]))
        triplets = [
            np.vstack([np.arange(0, l - 2), np.arange(1, l - 1), np.arange(2, l)]).T
            for l in lens
        ]
    ...
    def __getitem__(self, index):
        ...
        for ii in index:
            ...
            s1, s2, s3 = (
                self.transform(self.data[inds[0]]),
                self.transform(self.data[inds[1]]),
                self.transform(self.data[inds[2]]),
            )
...
def makeToyDataloaders(ds1, ds2, dt, batch_size=512, t="regular", nForward=1):
    ...
    trainDataLoader = DataLoader(
        dataset1, batch_size=adjustedBatch, shuffle=True, num_workers=4
    )
    testDataLoader = DataLoader(
        dataset2, batch_size=adjustedBatch, shuffle=False, num_workers=4
    )
