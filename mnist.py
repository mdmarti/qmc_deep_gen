...
def run_mnist_experiments(
    save_location, dataloc, train_grid_m=15, test_grid_m=20, n_recons=50, nEpochs=300
):
    ...
    train_loader = DataLoader(
        train_data, batch_size=256, shuffle=True, num_workers=n_workers
    )
    test_data = datasets.MNIST(dataloc, train=False, download=True, transform=transform)
    test_loader = DataLoader(
        test_data, batch_size=1, shuffle=False, num_workers=n_workers
    )
    ...
    if not os.path.isfile(save_qmc):
        ...
        qmc_model, qmc_opt, qmc_losses = train_qmc.train_loop(
            qmc_model,
            train_loader,
    ...
    model_grid_plot(
        ...
        n_samples_dim=20,
        show=False,
        fn=os.path.join(save_location, "qmc_grid.png"),
    )
    ...
    if not os.path.isfile(save_vae):
        ...
        vae_model, vae_opt, vae_losses = train_vae.train_loop(
            vae_model, train_loader, vae_loss_function, nEpochs=nEpochs
        )
    ...
    for ii in range(n_recons):
        ...
        save_fig = os.path.join(
            save_location, f"qmc_vae_round_trips_sample_{sample_ind}.png"
        )

        sample = (
            test_loader.dataset.data[sample_ind]
            .to(torch.float32)
            .to(device)
            .view(1, 1, 28, 28)
            / 256
        )
        recon_qmc1 = qmc_model.round_trip(
            test_base_sequence.to(device), sample, binary_lp
        )
        ...
        fig, axs = plt.subplots(
            nrows=1, ncols=3, figsize=(10, 5), sharex=True, sharey=True
        )
