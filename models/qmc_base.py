...
class FourierBasis(nn.Module):
    def __init__(self, num_dims=2, num_freqs=4, device=None):
        ...
        self.F = (
            ...
            * (
                torch.stack(
                    torch.meshgrid([torch.arange(num_freqs)] * num_dims, indexing="ij")
                ).reshape(num_dims, num_freqs**num_dims)
            ).to(device)
        )
    ...
    def forward(self, x):
        ...
        """
        return torch.hstack((torch.sin(x @ self.F), torch.cos(x @ self.F)))
...
class TorusBasis(nn.Module):
    ...
    def forward(self, data):
        ...
        return torch.cat(
            [torch.cos(2 * torch.pi * data), torch.sin(2 * torch.pi * data)], dim=-1
        )
...
class QMCLVM(nn.Module):
    def __init__(
        ...
        self.decoder = (
            nn.Sequential(
                ...
                ),  # nn.Linear(64*7*7,32*14*14),
                nn.ReLU(),
                nn.ConvTranspose2d(
                    32, 1, 3, stride=2, padding=1, output_padding=1
                ),  # nn.Linear(32*14*14,1*28*28),
                nn.Sigmoid(),
            ).to(device)
            if decoder is None
            else decoder.to(device)
        )
    ...
    def forward(self, eval_grid, random=True, mod=True, c=[]):
        ...
        r = (
            self.shift_function(1, self.latent_dim, device=self.device)
            if random
            else torch.zeros((1, self.latent_dim), device=self.device)
        )
    ...
    def posterior_probability(self, lattice, data, log_likelihood, grid_batch_size=-1):
        ...
        with torch.no_grad():
            ...
            for batch_on in range(0, B, grid_batch_size):
                ...
                model_lattice_lls.append(
                    log_likelihood(preds, data)
                )  # each entry A_ij is log p(x_i|z_j)
            model_lattice_lls = torch.cat(model_lattice_lls, axis=1)
            ## as such, model_lattice_lls should be n_data x n_grid points
            evidence = torch.special.logsumexp(
                model_lattice_lls, dim=1, keepdims=True
            ) - np.log(len(basis))  ## n_data x 1

            posterior = model_lattice_lls - evidence

            return nn.Softmax(dim=1)(
                posterior
            )  # posterior over grid points for each sample
        ...
        with torch.no_grad():
            if recon_type == "rqmc":
                ...
                for _ in range(n_samples):
                    tmp_grid = (
                        grid + torch.rand((1, grid.shape[1]), device=self.device)
                    ) % 1
                    posterior = self.posterior_probability(
                        tmp_grid, data, log_likelihood, grid_batch_size=grid_batch_size
                    )  # Bsz x Grid size
                    posterior_grid.append(
                        self.basis.reverse(
                            posterior.to(self.device) @ self.basis.forward(tmp_grid)
                        )
                    )  # Bsz x latent dim
                posterior_grid = self.basis.reverse(
                    self.basis.forward(torch.stack(posterior_grid, axis=0)).mean(axis=0)
                )
                ...
                for _ in range(n_samples):
                    ...
                    recons = torch.einsum(
                        "BG,GCHW->BCHW", posterior, recons
                    )  # posterior.to(self.device) @ recons
                    posterior_ims.append(recons)
                recon = torch.stack(posterior_ims, axis=0).mean(axis=0)

            else:
                posterior = self.posterior_probability(
                    grid, data, log_likelihood, grid_batch_size=grid_batch_size
                )
    ...
    def embed_data(
        ...
        with torch.no_grad():
            for data, label in tqdm(
                loader, desc="embedding latents", total=len(loader)
            ):
                ...
                if embed_type == "rqmc":
                    ...
                    latent_batch = self.basis.reverse(
                        self.basis.forward(torch.stack(latent_batch, axis=0)).mean(
                            axis=0
                        )
                    )  # Bsz x latent dim
                    ...
                    latents.append(
                        self.basis.reverse(posterior @ self.basis.forward(grid))
                        .detach()
                        .cpu()
                    )
                    ...
                    latents.append(
                        (grid[max_inds] % 1).detach().cpu()
                    )  # this may work? double check
