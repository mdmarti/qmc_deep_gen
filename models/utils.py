...
def get_decoder_arch(
    dataset_name, latent_dim, arch="qmc", n_per_sample=5, n_conditional=1
):
    ...
    if ("mnist_simple" in dataset_name.lower()) or (
        "binarized_mnist" in dataset_name.lower()
    ):
        ...
        layers = [
            ...
            nn.ConvTranspose2d(
                64, 64, 3, stride=2, padding=1, output_padding=1, groups=64
            ),  # nn.Linear(64*7*7,32*14*14),
            nn.Conv2d(64, 32, 1),
            ResCellNVAESimple(32, expand_factor=4, in_h=14, in_w=14),  # nn.ReLU(),
            nn.ConvTranspose2d(
                32, 32, 3, stride=2, padding=1, output_padding=1, groups=32
        ...
        decoder = nn.Sequential(nn.Linear(latent_dim, 500))
        layers = [nn.ReLU(), nn.Linear(500, 1000), nn.Unflatten(1, (1, 1, 1000))]
...
def get_encoder_arch(dataset_name, latent_dim, n_per_sample=5, diag=False):
    ...
    if "mnist_simple" in dataset_name.lower():
        ...
        encoder_net = nn.Sequential(
            nn.Flatten(start_dim=1, end_dim=-1), nn.Linear(28**2, 500), nn.Tanh()
        )
        ...
        mu_net = nn.Sequential(
            nn.Linear(n_per_sample * 100, 500), nn.ReLU(), nn.Linear(500, latent_dim)
        )
        ...
        encoder_net = nn.Sequential(
            nn.Flatten(start_dim=1, end_dim=-1), nn.Linear(1000, 500)
        )
        mu_net = nn.Linear(500, latent_dim)
