...
class LinearResBlock(nn.Module):
    def __init__(self, in_size, hidden_size=125):
        ...
        self.hidden = nn.Sequential(
            nn.BatchNorm1d(num_features=in_size),
...
class ConvResBlock(nn.Module):
    def __init__(
        self, in_channels, in_h, in_w, hidden_channels=32, kernel_size=3, pad=1
    ):

        super(ConvResBlock, self).__init__()

        self.hidden = nn.Sequential(
            nn.LayerNorm([in_channels, in_h, in_w]),
...
class MLPResCellNVAESimple(nn.Module):
    def __init__(self, in_size, expand_factor=4):
        ...
        self.ops = nn.Sequential(
            op1, op2, op3_1, op3_2, op4, op5_1, op5_2, op6, op7, op8
        )
...
class ResCellNVAESimple(nn.Module):
    def __init__(self, in_size, in_h, in_w, expand_factor=4):
        ...
        op3_2 = nn.SiLU()
        op4 = nn.Conv2d(
            expand_factor * in_size,
            expand_factor * in_size,
            5,
            groups=expand_factor * in_size,
            padding=2,
        )
...
class AVADecodeLayer(nn.Module):
    def __init__(self, in_channels, out_channels, H, W):
        ...
        self.net = nn.Sequential(
            ...
            nn.ConvTranspose2d(
                out_channels, out_channels, 3, 2, padding=1, output_padding=1
            ),
            nn.ReLU(),
        )
