...
def gen_korobov_basis(a, num_dims, num_points):
    """
...
def box_muller(unif_2d_vars):
    ...
    z1 = torch.sqrt(-2 * torch.log(unif_2d_vars[:, 0] + EPS)) * torch.cos(
        2 * torch.pi * unif_2d_vars[:, 1]
    )
    z2 = torch.sqrt(-2 * torch.log(unif_2d_vars[:, 0] + EPS)) * torch.sin(
        2 * torch.pi * unif_2d_vars[:, 1]
    )
...
def get_default_voronoi(grid):
    ...
    dists = cdist(
        pts[k][None, :], pts
    ).ravel()  # distance of all points to point closest to center of grid
    ...
    partition = np.argpartition(
        ...
        :n_neighbors
    ]  # 20 nearest neighbors to central point. This should be enough points, but can increase to 50 just in case
    neighbors = np.setdiff1d(
        partition, [k]
    )  # nearest neighbors not including center point

    vor = Voronoi(
        np.vstack([pts[k]] + [pts[kn] for kn in neighbors])
    )  # voronoi partition of central point and all neighbors
...
def sample_from_grid(grid, posterior, num_samples=1000):
    ...
    n = len(grid)
    samples = torch.multinomial(posterior, num_samples, replacement=True).to(
        grid.device
    )
    (B2, K) = samples.shape
    assert B1 == B2
    ...
    importance_weights = torch.mean(
        posterior_weights, axis=0, keepdims=True
    )  # is this correct here? yes
    # end = time.time() - start
    ...
    assert (importance_weights.shape[0]) == 1 and (
        importance_weights.shape[1] == B2 * K
...
def sample_from_cells(cell_centers, cell_bounds):
    ...
    dirichlet_samples = torch.from_numpy(
        gen.dirichlet(dirichlet_weights, size=(b, 1))
    ).to(cell_centers.device)
...
def gen_samples_batch(grid, model, lp, samples, n_samples_total):
    ...
    with torch.no_grad():
        posterior = model.posterior_probability(
            grid.to(model.device), samples.to(model.device), lp
        )

        centered_cell = get_default_voronoi(grid)

        sampled_shifts, importance_weights = sample_from_grid(
            grid, posterior, num_samples=n_per_s
        )
