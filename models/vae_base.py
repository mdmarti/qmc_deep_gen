...
class VAE(nn.Module):
    ...
    def posterior(self, target, n_quadrature_points, log_prob_fnc):
        ...
        log_prior = torch.from_numpy(weights_2d.flatten()).to(
            self.device
        )  ## these are approoooximately log p(z), (2500,)
        xy_grid = (
            torch.from_numpy(np.stack([x_grid.flatten(), y_grid.flatten()], axis=1))
            .to(torch.float32)
            .to(self.device)
        )  # (2500,2)
        log_likelihood = log_prob_fnc(
            self.decoder(xy_grid), target
        )  ## these are approooooooxxxxximately log p(x|z) (2500,1)
        ...
        log_evidence = torch.special.logsumexp(
            log_likelihood + log_prior[:, None], dim=0
        )  # this is approoooooooooooxxximately log p(x) #(1,)
        log_posterior = (
            log_likelihood.squeeze() + log_prior.squeeze() - log_evidence.squeeze()
        )  # (2500,)
        return (
            nn.Softmax(dim=0)(log_posterior).detach().cpu().numpy().squeeze(),
            nn.Softmax(dim=0)(lp_grid_encoder).detach().cpu().numpy(),
            xy_grid.detach().cpu().numpy(),
        )
    ...
    def embed_data(self, loader):
        ...
        with torch.no_grad():
            for data, label in tqdm(
                ...
                if type(label) == tuple:
                    labels.append(
                        [string.ascii_lowercase.index(l.lower()[0]) for l in label]
                    )
                else:
...
class IWAE(VAE):
    ...
    def forward(self, x):
        ...
        recons = torch.vmap(self.decoder, in_dims=(1), out_dims=(1))(z).permute(
            1, 0, 2, 3, 4
        )  # KxBxCxHxW->BxKxCxHxW
